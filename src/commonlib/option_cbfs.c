...
#define fail(...) (printk(__VA_ARGS__), CB_CMOS_OPTION_NOT_FOUND)
...
enum cb_err get_option(void *dest, const char *name)
{
	...
	boot_file = map_cbfs_file(
		BOOTORDER_FILE, CBFS_TYPE_RAW, &boot_file_len
	);

	if (boot_file == NULL)
		return fail(BIOS_ALERT,
			"file [%s] not found in CBFS\n", BOOTORDER_FILE
		);

	if (boot_file_len < 4096)
		return fail(BIOS_ALERT, "Missing bootorder data.\n");
