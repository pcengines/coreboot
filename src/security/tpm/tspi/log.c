...
static struct tcpa_table *tcpa_log_init(void)
{
	...
	if (!cbmem_possibly_online() &&
		IS_ENABLED(CONFIG_VBOOT_TCPA_LOG_RECOVERY))
		return (struct tcpa_table *)_vboot2_tpm_log;
	else if (ENV_ROMSTAGE && IS_ENABLED(CONFIG_VBOOT_TCPA_LOG_RECOVERY)) {
...
void tcpa_log_add_table_entry(const char *name, const uint32_t pcr,
			const char *digest_type, const uint8_t *digest,
			const size_t digest_len)
{
	...
	if (!name || !digest_type) {
		printk(BIOS_WARNING, "TCPA: TCPA entry name or digest type not set\n");
		return;
	}
	...
	strncpy(tce->digest_type, digest_type, TCPA_PCR_HASH_LEN - 1);
	tce->digest_length = MIN(digest_len, TCPA_DIGEST_MAX_LENGTH);
	memcpy(tce->digest, digest, tce->digest_length);
}

void tcpa_preram_log_clear(void)
{
	if (ENV_CACHE_AS_RAM && IS_ENABLED(CONFIG_VBOOT_STARTS_IN_BOOTBLOCK)
	    && IS_ENABLED(CONFIG_VBOOT_TCPA_LOG_RECOVERY)) {
		printk(BIOS_INFO, "TCPA: Clearing coreboot TCPA log\n");
		struct tcpa_table *tclt = (struct tcpa_table *)_vboot2_tpm_log;
		if (!tclt) {
			printk(BIOS_ERR, "TCPA: Couldn't clear coreboot TCPA log\n");
			return;
		}
		tclt->max_entries = MAX_TCPA_LOG_ENTRIES;
		tclt->num_entries = 0;
...
static void recover_tcpa_log(int is_recovery)
{
	...
	if (preram_log->num_entries > MAX_PREMEM_TCPA_LOG_ENTRIES) {
